...
uchar Recive_table[86]; //用于接收wifi模块反馈到MCU上的数据
	  ...
	  //发送订阅指令，原格式为：
		...
		{        
				...
				if(flag == 1){
					...
					else if(strstr(Recive_table,"&msg=off"))
					{ ////当检测到字符串msg=off时，执行关灯
					...
					else if(strstr(Recive_table,"&msg=led1on"))
					{ ////当检测到字符串msg=led1on时，执行开白灯
							    WhileLed=0;
								Ledflash(2);
								ms_delay(10);	
					}
					else if(strstr(Recive_table,"&msg=led1off"))
					{ ////当检测到字符串msg=led1off时，执行关白灯
								WhileLed=1;
								Ledflash(5);
								ms_delay(10);
					}
					else if(strstr(Recive_table,"&msg=led2on"))
					{ ////当检测到字符串msg=off时，执行开蓝灯
								BuleLed=0;
								Ledflash(1);
								ms_delay(10);	
					}
					else if(strstr(Recive_table,"&msg=led2off"))
					{ ////当检测到字符串msg=led2off时，执行关蓝灯
								BuleLed=1;
								Ledflash(4);
								ms_delay(10);
					}
					else if(strstr(Recive_table,"&msg=show"))
					{ ////当检测到字符串msg=show时，执行灯光秀
								//BuleLed=1;
								Ledflash(7);
								WaterLamp();
					...
					else if(strstr(Recive_table,"&msg=alloff"))
					{ ////当检测到字符串msg=alloff时，关闭全部灯
								//BuleLed=1;
								P1=0xff;	//关闭灯，P10引脚置低电平
								Ledflash(8);
								ms_delay(10);
					}
					else if(strstr(Recive_table,"ERROR"))
					{ //如果掉线或网络故障，重新连接
									connected = 1;
									SerialSend("AT+CIPMODE=1"); //开启透明传输模式
									ms_delay(1000);
